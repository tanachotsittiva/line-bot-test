...
def get_relevant_laws(query):
    ...
    try:
        ...
        # เพิ่มความเข้มงวดของคะแนน (Threshold) จาก 0.1 เป็น 0.2
        results = [LAWS_LIST[i] for i in top_indices if cosine_sim[0][i] > 0.2]
        return "\n\n".join(results)
    except:
        return ""
...
def handle_text(event):
    ...
    system_prompt = (
        ...
        "3. ใช้เฉพาะข้อกฎหมายที่แนบไปให้เท่านั้นในการระบุเลขมาตรา 'ห้าม' สร้างเลขมาตราหรือเนื้อหากฎหมายขึ้นมาเองเด็ดขาด,หากในข้อมูลไม่มี ให้ตอบว่า 'เรื่องนี้หมวดยังไม่มีข้อมูลแน่ชัดครับ' ให้แนะนำให้ติดต่อ สน. ใกล้บ้าน\n  📄 "
        "4. หากเป็นคำถามทั่วไป หรือการแจ้งเหตุฉุกเฉิน ที่ไม่มีในไฟล์กฎหมาย ให้ใช้ความรู้ทั่วไปของตำรวจในการตอบและแนะนำอย่างสุภาพ✨\n"
        "5. หากประชาชนถามหาโรงพัก ให้แนะนำว่า 'กดปุ่ม (+)แล้วเลือก ตำแหน่งที่ตั้ง (Location)ส่งมาให้หมวดได้เลยครับ' 📍"
    )   
